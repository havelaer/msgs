...
export default msgs.parse({
	...
	functions: {
		date: {
			"en-US":
				"Date: {$date :date}, Time: {$date :time}, Date and Time: {$date :datetime}",
			"nl-NL":
				"Datum: {$date :date}, Tijd: {$date :time}, Datum en Tijd: {$date :datetime}",
			"fr-FR":
				"Date: {$date :date}, Heure: {$date :time}, Date et Heure: {$date :datetime}",
		},
		percent: {
